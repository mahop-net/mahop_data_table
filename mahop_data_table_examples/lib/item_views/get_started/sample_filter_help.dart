...
You can search for first name, last name and street

### Things of interest:
- Custom filter function
- In this sample you can type e.g. firstname and lastname with a space in between to filter for both values with logical AND
- this sample filters for first and last name, street and city, but this is completely customizable
...
class _SampleFilterState extends State<SampleFilter> {
  ...
  void initState() {
    ...
    //Define the Columns
    columnDefs.add(MhItemsViewColumnDef<RandomAddress>(id: "firstName", header: "First name", columnWidth: 110, getDisplayValue: (item) => item.firstName));
    columnDefs.add(MhItemsViewColumnDef<RandomAddress>(id: "lastName", header: "Last name", columnWidth: 150, getDisplayValue: (item) => item.lastName));
    columnDefs.add(MhItemsViewColumnDef<RandomAddress>(id: "street", header: "Street", columnWidth: 150, getDisplayValue: (item) => item.street));
    columnDefs.add(MhItemsViewColumnDef<RandomAddress>(id: "state", header: "State", columnWidth: 50, getDisplayValue: (item) => item.state));
    columnDefs.add(MhItemsViewColumnDef<RandomAddress>(id: "zipCode", header: "Zip", columnWidth: 40, getDisplayValue: (item) => item.zipCode));
    columnDefs.add(MhItemsViewColumnDef<RandomAddress>(id: "city", header: "City", columnWidth: 120, getDisplayValue: (item) => item.city));
    columnDefs.add(MhItemsViewColumnDef<RandomAddress>(id: "telephoneNumber", header: "Phone", columnWidth: 150, getDisplayValue: (item) => item.telephoneNumber));
    columnDefs.add(MhItemsViewColumnDef<RandomAddress>(id: "mobileTelephoneNumber", header: "Mobile phone", columnWidth: 150, getDisplayValue: (item) => item.mobileTelephoneNumber));
  ...
  Widget build(BuildContext context) {
    ...
    final mhItemsView = MhItemsView<RandomAddress>(itemsSource: items, columnDefs: columnDefs, settings: settings);
                  ...
                  onChanged: (filterValue) {
                    mhItemsView.setFilter((item) {
                      ...
                      for (var filterToken in filterTokens) {
                        var found = false;
                        if (item.firstName.toLowerCase().contains(filterToken)) {
                          found = true;
                        }
